...
def get_input_path() -> Path:
    ...
    return Path(questionary.path(
        "Enter the path to the input directory"
    ).ask())
...
def get_output_path(input_path: Path) -> Path:
    output_path = Path(questionary.path(
...
def verify_channel_list(channel_list: list[str], output_path: Path):
    ...
    overwrite_candidates = [output_file_path for output_file_path in channel_list if (output_path / output_file_path).exists()]
    if overwrite_candidates:
        error_message_lines = ["", "The following channel names would overwrite existing files in the output directory:"]
...
def choose_channels_to_extract(channel_list: list[str]) -> list[int]:
    return questionary.checkbox(
        ...
        validate=lambda selected: True if selected else "You must select at least one channel"
    ).ask()
...
def main():
    ...
    with wave.open(str(wav_paths[0]), 'rb') as wf:
        ...
        if channel_count < len(channel_list):
            print(f"{len(channel_list)} channel names provided, but source only has {channel_count} channels")
            ...
            print(f"Source has {channel_count} channels, but only {len(channel_list)} channel names provided")
        ...
        for wav_path in wav_paths:
            ...
            with wave.open(str(wav_path), 'rb') as wf:
                ...
                while True:
                    ...
                    frames = wf.readframes(2 ** 16) # tune for performance/memory tradeoff
                    if not frames:
                        break
    ...
    verify_output_files(channel_list, output_path, total_frames, sampwidth, channels_to_extract)
